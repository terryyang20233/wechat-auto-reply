...
在「回复备选」面板（以及设置）里，先选模式，再选策略：

| 模式 | 策略 |
|---|---|
| 日常 | 最正常的微信口吻（默认） |
| 恋爱/暧昧 | 推拉俏皮 / 情绪价值 / 延展话题 |
| 职场/工作 | 专业高效 / 委婉拒绝或延后 / 请示确认 |
| 怼人/防守 | 阴阳怪气（不带脏字） / 礼貌保持距离 |
...
- 聊天内容默认 **不写日志、不落库**。
- 发给 AI 前会把发言者角色化为「我 / 对方 / 成员N」，并去掉明显手机号等标识。设置里可以关掉隐藏姓名；建议保持开启。
...
- 在「系统设置 → 隐私与安全性 → 辅助功能」中添加 **微信回复助手**（`/Applications/微信回复助手.app`）；若从虚拟环境直接跑，还要添加 **Python**（通常是 `Python.app`，例如 python.org 安装的  
  `/Library/Frameworks/Python.framework/Versions/3.12/Resources/Python.app`）。  
  只勾选 Cursor / 终端不够。添加后必须重启助手。

## 启动

双击 `/Applications/微信回复助手.app`。第一次会装依赖并打开浏览器；从程序坞退出助手时会关掉后台服务。
...
- 微信改版后，辅助功能树可能变化。若读不到消息，在本机打开 `/api/diagnostics/ax` 查看控件树，再针对性适配。  
